...
<div class="w-full px-6 py-6 mx-auto">
    <div class="bg-white shadow-lg rounded-2xl p-6 border border-slate-200">
        ...
        {{-- Table (Sesuai Acuan: border border-slate-200 shadow rounded-2xl bg-white) --}}
                    ...
                    <tbody>
                        ...
                        @forelse ($trialBalance as $row)
                            ...
                            <tr class="border-b hover:bg-slate-50">
                                ...
                                <td class="px-6 py-3 border border-slate-200 text-right">Rp {{ number_format($row->debit, 0, ',', '.') }}</td>
                                <td class="px-6 py-3 border border-slate-200 text-right">Rp {{ number_format($row->kredit, 0, ',', '.') }}</td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="4" class="text-center py-4 text-slate-500">Tidak ada data untuk periode ini</td>
                            </tr>
                        @endforelse

                        @if (isset($trialBalance) && $trialBalance->count() > 0)
                            <tr class="bg-slate-100 font-bold border-t">
                                ...
                                <td class="px-6 py-3 border border-slate-200 text-right">Rp {{ number_format($totalDebit, 0, ',', '.') }}</td>
                                <td class="px-6 py-3 border border-slate-200 text-right">Rp {{ number_format($totalKredit, 0, ',', '.') }}</td>
                            </tr>
                            @if($totalDebit !== $totalKredit)
                                <tr class="bg-red-50 text-red-700 font-bold">
                                    <td colspan="4" class="px-6 py-3 text-center">
                                        Neraca tidak seimbang! Selisih: Rp {{ number_format(abs($totalDebit - $totalKredit), 0, ',', '.') }}
                                    </td>
                                </tr>
                            @endif
                        @endif
